...
The initial task steps below record the original input-size hypothesis. Later live testing showed two separate issues: exact `OK`/low was not a reliable first workload, and server renewal could appear only after Gudmo's first verification read. The remaining floating account renewed with 128 output words at medium reasoning. Production now starts with that observed working profile, polls metadata for delayed propagation without another model call, and retains only 256/512-word high-reasoning fallbacks. The final configuration is defined in `src/prompt.js`, covered by `test/prompt.test.js` and `test/scheduler.test.js`, and reflected in the updated design spec and README. Where the original task samples below mention five tiers, input payload growth, exact `OK`, or the earlier cheap-first ladder, this amendment supersedes them.

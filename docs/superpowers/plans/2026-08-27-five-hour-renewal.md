...
The initial task steps below record the original input-size hypothesis. Later live testing showed two separate issues: cheap profiles were not reliable across accounts, and server renewal could appear only after Gudmo's first verification read. A 128-word/medium request renewed one account but left the problematic account floating after 14,942 total tokens; 256 words/high renewed that account with 2,146 output tokens. Production now starts with that observed working profile, polls metadata for delayed propagation without another model call, and retains one 512-word/high fallback. The final configuration is defined in `src/prompt.js`, covered by `test/prompt.test.js` and `test/scheduler.test.js`, and reflected in the updated design spec and README. Where the original task samples below mention five tiers, input payload growth, exact `OK`, or the earlier cheap-first ladder, this amendment supersedes them.

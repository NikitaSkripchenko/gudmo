...
`gudmo` is a manual macOS CLI that ensures the Codex **five-hour usage-window timer** is
active for every locally discovered account. It renews only accounts whose window has not
started yet.
...
- External interfaces: Codex CLI `codex exec --json` events, and Codex app-server
  JSON-RPC over `stdio://` for rate-limit metadata.
...
- At most **two** prompt attempts per account (one per tier), escalating only after failed
  verification.
...
| `src/cli.js` | Argument parsing, command dispatch, parallel per-account execution, progress and result formatting, aggregate exit code. |
...
| `src/prompt.js` | Pure prompt builder `buildRenewalPrompt({ tier, nonce })` and `PROMPT_TIERS`. |
| `src/accounts.js` | `codex-auth` registry discovery, isolated `CODEX_HOME` preparation, per-account paths. |
...
Each `gudmo run` reads fresh metadata for every discovered account. Per account:
...
Supported config keys: `codexPath`, `model`, `reasoningEffort`, `timeoutSeconds`,
`retrySeconds`. Legacy scheduler, window, request-ceiling, and custom-message keys are
...
malformed telemetry, credential isolation, and `eval` using the production prompt builder.
